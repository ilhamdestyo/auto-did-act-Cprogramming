...
//variable operasi pada calculator
gint numchooser = 0; 
gint num1 = 0, num2 = 0;
gchar op;
...
//mainWindow
GtkWidget *window;
GtkWidget *cek_order_btn, *cek_calc_btn;
...
GtkWidget *main_label, *main_labelcalc, *bt_lb1, *bt_lb2, *bt_lb3, *bt_lb4, *bt_lb5, *bt_lb6, *bt_lb7, *bt_lb8, *bt_lb9, *bt_lb10;
...
//windowCalculator
GtkWidget *windowCalc;
GtkWidget *calc_boxC, *calc_grid_num, *calc_numBox;
GtkWidget *calc_close_btn, *calc_num_bt1, *calc_num_bt2, *calc_num_bt3, *calc_num_bt4, *calc_num_bt5, *calc_num_bt6, *calc_num_bt7, *calc_num_bt8, *calc_num_bt9, *calc_num_bt0, *calc_num_btclear, *calc_num_btenter, *calc_num_btsum, *calc_num_btsubstraction, *calc_num_btmultiple, *calc_num_btdivision;
GtkWidget *calc_number;
...
static void start_windowCalc();//tambahan start calculator
...
static void view_windowCalc();//tambahan view calculator
...
//tambahan fungsi operasi algoritma kalkulator
void gtk_windowCalc_init();
void gtk_windowCalc_setAttrib();
void gtk_windowCalc_connect();
static void close_windowCalc();
static void calc_setNumber(GtkWidget *numWidget);
static void calc_sum(GtkWidget *numWidget);
static void calc_substraction(GtkWidget *Numwidget);
static void calc_multiple(GtkWidget *Numwidget);
static void calc_division(GtkWidget *Numwidget);
static void calc_result(GtkWidget *Numwidget);
...
int main(int argc, char **argv){
	...
	start_windowNumi();
	start_windowMpb();
	start_windowCalc();//tambahan window start kalkulator
	//start
...
void gtk_mainWindow_init(){
	...
	cek_calc_btn=GTK_WIDGET(gtk_builder_get_object(builder, "cek_calc_btn"));//panggil button calc pada main window
	...
	main_labelcalc=GTK_WIDGET(gtk_builder_get_object(builder, "main_labelcalc"));
...
void gtk_mainWindow_setAttrib(){
	...
	gtk_widget_set_name (cek_calc_btn, "cek_calc_btn");//calculator
	...
	gtk_widget_set_name(main_labelcalc, "main_labelcalc");//label kalculator
	...
	gtk_widget_set_size_request(main_labelcalc, lbc_width, lbc_hight);
...
void gtk_mainWindow_connect(){
	...
	g_signal_connect(cek_calc_btn, "clicked", G_CALLBACK(view_windowCalc),NULL);
}
...
static void start_windowCalc(){
	...
	gtk_windowCalc_init();
	gtk_windowCalc_setAttrib();
	gtk_windowCalc_connect();

	gtk_builder_connect_signals(builder, NULL);
	gtk_widget_hide(windowCalc);
}
...
//view kalkulator
static void view_windowCalc(){
	gtk_widget_show(windowCalc);
}
...
void gtk_windowNumi_setAttrib(){
	...
	strcpy(markup_text, "<span foreground='#777777'><b>Input Number</b></span>");
...
static void numi_setNumber(GtkWidget *numWidget){
	...
	if (strlen(input_number_text)<12 || myNumber[0]=='D'){
		...
		if(strlen(input_number_text)>0){
			...
			strcpy(markup_text, g_markup_printf_escaped (format, "Input Number"));
		}
		gtk_label_set_markup(GTK_LABEL(numi_number), (const gchar *) markup_text);
	}
}
...
//windowCalc
void gtk_windowCalc_init(){
	windowCalc=GTK_WIDGET(gtk_builder_get_object(builder, "windowCalc"));
	
	calc_boxC=GTK_WIDGET(gtk_builder_get_object(builder, "calc_boxC"));
	calc_grid_num=GTK_WIDGET(gtk_builder_get_object(builder, "calc_grid_num"));
	calc_numBox=GTK_WIDGET(gtk_builder_get_object(builder, "calc_numBox"));
	
    calc_num_bt1=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt1"));
	calc_num_bt2=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt2"));
	calc_num_bt3=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt3"));
	calc_num_bt4=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt4"));
	calc_num_bt5=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt5"));
	calc_num_bt6=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt6"));
	calc_num_bt7=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt7"));
	calc_num_bt8=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt8"));
	calc_num_bt9=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt9"));
	calc_num_bt0=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_bt0"));
	calc_num_btclear=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_btclear"));
	calc_num_btenter=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_btenter"));
	calc_num_btsum=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_btsum"));
	calc_num_btsubstraction=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_btsubstraction"));
	calc_num_btmultiple=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_btmultiple"));
	calc_num_btdivision=GTK_WIDGET(gtk_builder_get_object(builder, "calc_num_btdivision"));
	calc_close_btn=GTK_WIDGET(gtk_builder_get_object(builder, "calc_close_btn"));
	
	calc_number=GTK_WIDGET(gtk_builder_get_object(builder, "calc_number"));

	memset(input_number_text, 0x00, 20*sizeof(char));
}

void gtk_windowCalc_setAttrib(){
	gtk_window_set_position(GTK_WINDOW(windowCalc), GTK_WIN_POS_CENTER_ALWAYS);

	gtk_widget_set_name(windowCalc, "windowCalc");

	gtk_widget_set_name(calc_grid_num, "calc_grid_num");
	gtk_widget_set_name(calc_numBox, "calc_numBox");

	gtk_widget_set_name(calc_num_bt1, "calc_num_bt1");
	gtk_widget_set_name(calc_num_bt2, "calc_num_bt2");
	gtk_widget_set_name(calc_num_bt3, "calc_num_bt3");
	gtk_widget_set_name(calc_num_bt4, "calc_num_bt4");
	gtk_widget_set_name(calc_num_bt5, "calc_num_bt5");
	gtk_widget_set_name(calc_num_bt6, "calc_num_bt6");
	gtk_widget_set_name(calc_num_bt7, "calc_num_bt7");
	gtk_widget_set_name(calc_num_bt8, "calc_num_bt8");
	gtk_widget_set_name(calc_num_bt9, "calc_num_bt9");
	gtk_widget_set_name(calc_num_bt0, "calc_num_bt0");
	gtk_widget_set_name(calc_num_btclear, "calc_num_btclear");
	gtk_widget_set_name(calc_num_btenter, "calc_num_btenter");
	gtk_widget_set_name(calc_num_btsum, "calc_num_btsum");
	gtk_widget_set_name(calc_num_btsubstraction, "calc_num_btsubstraction");
	gtk_widget_set_name(calc_num_btmultiple, "calc_num_btmultiple");
	gtk_widget_set_name(calc_num_btdivision, "calc_num_btdivision");
	gtk_widget_set_name(calc_close_btn, "calc_close_btn");

	gtk_widget_set_name(calc_number, "calc_number");

	gtk_style_context_add_provider_for_screen(gdk_screen_get_default(), GTK_STYLE_PROVIDER(cssProvider), GTK_STYLE_PROVIDER_PRIORITY_USER);

	char markup_text[200];
	strcpy(markup_text, "<span foreground='#777777'><b>Input Number</b></span>");
	gtk_label_set_markup(GTK_LABEL(calc_number), (const gchar *) markup_text);

	//set dimention
	int calc_margin = screenWidth/60;
	int calc_boxC_space = screenWidth/40;
	int calc_bt_dimention = screenWidth/19;
	int calc_grid_num_space = screenWidth/80;

	gtk_widget_set_margin_start(calc_boxC, calc_margin);
	gtk_widget_set_margin_end(calc_boxC, calc_margin);
	gtk_widget_set_margin_top(calc_boxC, calc_margin);
	gtk_widget_set_margin_bottom(calc_boxC, calc_margin);
	gtk_box_set_spacing(GTK_BOX(calc_boxC), calc_boxC_space);

	gtk_grid_set_row_spacing(GTK_GRID(calc_grid_num), calc_grid_num_space);
	gtk_grid_set_column_spacing(GTK_GRID(calc_grid_num), calc_grid_num_space);
	gtk_widget_set_margin_top(calc_grid_num, calc_margin);
	gtk_widget_set_margin_bottom(calc_grid_num, calc_margin);

	gtk_widget_set_size_request(calc_num_bt1, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_bt2, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_bt3, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_bt4, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_bt5, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_bt6, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_bt7, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_bt8, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_bt9, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_bt0, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_btclear, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_btenter, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_btsum, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_btsubstraction, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_btmultiple, calc_bt_dimention, calc_bt_dimention);
	gtk_widget_set_size_request(calc_num_btdivision, calc_bt_dimention, calc_bt_dimention);

	gtk_widget_set_size_request(calc_close_btn, calc_bt_dimention/2, calc_bt_dimention/2);
}

void gtk_windowCalc_connect(){
	g_signal_connect(windowCalc, "destroy", G_CALLBACK(gtk_main_quit), NULL);

	g_signal_connect(calc_num_bt1, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_bt2, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_bt3, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_bt4, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_bt5, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_bt6, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_bt7, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_bt8, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_bt9, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_bt0, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_btclear, "clicked", G_CALLBACK(calc_setNumber), NULL);
	g_signal_connect(calc_num_btenter, "clicked", G_CALLBACK(calc_result), NULL);
	g_signal_connect(calc_num_btsum, "clicked", G_CALLBACK(calc_sum), NULL);
	g_signal_connect(calc_num_btsubstraction, "clicked", G_CALLBACK(calc_substraction), NULL);
	g_signal_connect(calc_num_btmultiple, "clicked", G_CALLBACK(calc_multiple), NULL);
	g_signal_connect(calc_num_btdivision, "clicked", G_CALLBACK(calc_division), NULL);
	g_signal_connect(calc_close_btn, "clicked", G_CALLBACK(close_windowCalc), NULL);
}
...
static void calc_setNumber(GtkWidget *numWidget){
	...
	strcpy(myNumber, (const char *)gtk_button_get_label(GTK_BUTTON(numWidget)));
	if (strlen(input_number_text)<12 || myNumber[0]=='C'){
		if(myNumber[0]!='C'){
			input_number_text[strlen(input_number_text)]=myNumber[0];
			input_number_text[strlen(input_number_text)]=0;
		}
...
static void calc_sum(GtkWidget *Numwidget){
		op = '+'; //set operator +
		if(numchooser == 0){	
			num1 = atoi(input_number_text);
			g_print("num1 : %d\n", num1);
			memset(input_number_text, 0x00, 20*sizeof(char));
			numchooser = 1;
		}else if(numchooser == 1){
			num2 = atoi(input_number_text);
			g_print("num2 : %d\n", num2);
			memset(input_number_text, 0x00, 20*sizeof(char));
			numchooser = 0;
		}
...
static void calc_result(GtkWidget *Numwidget){		
		if(numchooser == 0){	
			num1 = atoi(input_number_text);//string to integer
			g_print("num1 : %d\n", num1); 
			memset(input_number_text, 0x00, 20*sizeof(char));
			numchooser = 1;
		}else if(numchooser == 1){
	    ...
	    case 'x':
	      ...
	      break;
	    case '/':
	      resultflt = (float)num1 / num2;
	      break;
	    default:
	      g_error("Error! operator is not correct");
	  }
		...
		char resultstr[200];
		char markup_text[200];
		if(op == '/') sprintf(resultstr, "%.2f", resultflt);
		else sprintf(resultstr, "%d", result);
		
		strcpy(format, "<span foreground='#aa5533'><b>\%s</b></span>");
		strcpy(markup_text, g_markup_printf_escaped (format, resultstr));		
		gtk_label_set_markup(GTK_LABEL(calc_number), (const gchar *) markup_text);
}

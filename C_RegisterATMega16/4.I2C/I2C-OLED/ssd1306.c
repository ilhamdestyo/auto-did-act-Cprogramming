...
uint8_t SSD1306_Send_StartAndSLAW (uint8_t address)
{
  ...
  // TWI: send SLAW
  // -------------------------
  status = I2C_MT_Send_SLAW (address);
  // request - send SLAW
  if (SSD1306_SUCCESS != status) {
    // error
    return status;
...
uint8_t SSD1306_Send_Command (uint8_t command)
{
  ...
  // send control byte
  // -------------------------    
  status = I2C_MT_Send_Data (SSD1306_COMMAND);
  // request - start TWI
  if (SSD1306_SUCCESS != status) {
    // error
    return status;
  ...
  // send command
  // -------------------------    
  status = I2C_MT_Send_Data (command);
  // request - start TWI
  if (SSD1306_SUCCESS != status) {
    // error
    return status;
...
uint8_t SSD1306_UpdateScreen (uint8_t address)
{
  ...
  // -------------------------    
  status = I2C_MT_Send_Data (SSD1306_DATA_STREAM);
  // request succesfull
  if (SSD1306_SUCCESS != status) {
    // error
    return status;
  ...
  while (i < CACHE_SIZE_MEM) {
    // send data
    status = I2C_MT_Send_Data (cacheMemLcd[i]);
    // request succesfull
    if (SSD1306_SUCCESS != status) {
      // error
      return status;

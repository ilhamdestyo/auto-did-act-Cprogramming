 ...
 *dibuat 2 variable data raw sebagai data input
 *
 */ 
...
int main(int argc, char *argv[]){
	...
	for(int i=1;i<number_data+1;i++){
		count++;
		dataBaca.Input1[count]=IntToChar(datainput_1[i]);
	}
	
	for(int i=1;i<number_data+1;i++){
		count++;
		dataBaca.Input1[count]=IntToChar(datainput_2[i]);
	}
	...
	for(int i=1;i<dataBaca.ndata+1;i++){
		...
		dataBaca.Input2[i]="";
		char *number = IntToStr(i);
		strcat (dataBaca.Input1[i],number);
	}
	
	save_data_file("data_real_copy.txt",dataBaca);

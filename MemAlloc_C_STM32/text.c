...
int main(void){
	FCPU = 1; //in MHz
	DDRD = (1 << (bit0)) | (1<<(bit1)) | (1<<(bit2)) | (1<<(bit3)) | (1<<(bit4)) | (1<<(bit5)) | (1<<(bit6)) | (1<<(bit7)) ; //give initial value 1 to DDRA bit0, configure as output
	//DDRD |= (1 << (bit7)); //give value 1 to DDRD bit7, without destructing previous value of DDRD, configure as output
	
    for(;;) //do infinite loop
    {
        PORTD ^= 0xFF; //give value 1 to bit0, toggles the state of that pin every 5 ms (if previous value is 1, then next value is 0 and so on)
        delayms(100);
    }
	return 0;
}
